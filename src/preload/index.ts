...
const api: NightShiftApi = {
  ...
  planner: {
    ...
    deleteQueuedTask: (taskId) => invoke(IPC_CHANNELS.plannerDeleteQueuedTask, { taskId }),
    updateQueuedPriority: (taskId, priority) => invoke(IPC_CHANNELS.plannerUpdateQueuedPriority, { taskId, priority }),
    purgeTask: (taskId) => invoke(IPC_CHANNELS.plannerPurgeTask, { taskId }),
    ...
    getQueueState: () => invoke(IPC_CHANNELS.plannerGetQueueState, undefined),
    setQueueState: (paused) => invoke(IPC_CHANNELS.plannerSetQueueState, { paused }),
  },
  runs: {
    list: (workspaceId) => invoke(IPC_CHANNELS.runsList, { workspaceId }),
    navigation: (workspaceId) => invoke(IPC_CHANNELS.runsNavigation, { workspaceId }),
