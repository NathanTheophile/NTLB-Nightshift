...
export const registerIpcHandlers = (services: IpcServices): void => {
  ...
  handle(IPC_CHANNELS.runsRetryIntegration, async (request) => { assertRecord(request); assertNonEmptyString(request.runId, 'runId'); return services.reviewIntegration.retryIntegration(request.runId); });
  handle(IPC_CHANNELS.runsGetCandidateProgression, (request) => { assertRecord(request); assertNonEmptyString(request.workspaceId, 'workspaceId'); return { workspaceId: request.workspaceId, mode: services.reviewIntegration.getCandidateProgressionMode(request.workspaceId) }; });
  handle(IPC_CHANNELS.runsSetCandidateProgression, (request) => { assertRecord(request); assertNonEmptyString(request.workspaceId, 'workspaceId'); if (request.mode !== 'candidate_only' && request.mode !== 'auto_review' && request.mode !== 'auto_review_integrate') throw new Error('Unknown candidate progression mode.'); return { workspaceId: request.workspaceId, mode: services.reviewIntegration.setCandidateProgressionMode(request.workspaceId, request.mode) }; });
