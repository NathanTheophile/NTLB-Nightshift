...
export class ProjectValidationService {
  ...
  public async validate(runId: string, worktreePath: string, options: ValidationExecutionOptions): Promise<ValidationStatus> {
    ...
    for (const spec of commands) {
      ...
      const result = await this.executeScript(runId, worktreePath, spec.script, options);
  ...
  private async executeScript(runId: string, workingDirectory: string, script: string, options: ValidationExecutionOptions): Promise<{ exitCode: number | null; output: string; truncated: boolean; interruption: 'cancelled' | 'timed_out' | null }> {
    const executionId = `validation-${runId}-${randomUUID()}`;
    const executablePath = process.platform === 'win32' ? 'cmd.exe' : 'npm'; const arguments_ = process.platform === 'win32' ? ['/d', '/s', '/c', `npm run ${script}`] : ['run', script];
...
const resolveCommands = async (worktreePath: string): Promise<ValidationCommand[]> => {
  try {
    ...
    return scripts && typeof scripts === 'object' ? SCRIPT_ORDER.filter((script) => typeof scripts[script] === 'string' && scripts[script].trim()).map((script) => ({ script, command: `npm run ${script}` })) : [];
  } catch { return []; }
};
