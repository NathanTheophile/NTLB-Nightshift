...
import type { DelegatedLeaderClient, LeaderDecision, LeaderRequest } from './DelegatedLeaderClient';
...
export class DelegatedRunOrchestrator {
  public constructor(private readonly runs: RunRepository, private readonly validation: ProjectValidationService, private readonly reviews: RunReviewService, private readonly leader: DelegatedLeaderClient) {}
  ...
  public async execute(spec: { run: Run; task: PlannerTask; worktreePath: string; adapter: AgentAdapter; deadline: number; isCancellationRequested: () => boolean; setActive: (cancel: () => Promise<void>) => void; clearActive: () => void }): Promise<RunStatus> {
    ...
    const luna = await this.leader.resolveLuna(); this.runs.setDelegatedMetadata(run.id, luna.id, MAX_ATTEMPTS, 'planning'); this.runs.appendEvent(run.id, 'leader_resolution', { modelId: luna.id });
  ...
  private async worker(spec: Parameters<DelegatedRunOrchestrator['execute']>[0], attemptId: string, prompt: string): Promise<AgentExecutionResult> {
    let handle: Awaited<ReturnType<AgentAdapter['startWorker']>> | undefined;
    try { handle = await spec.adapter.startWorker({ workerId: attemptId, workspaceId: spec.run.workspaceId, workingDirectory: spec.worktreePath, modelId: spec.run.resolvedModelId, permissionProfile: 'isolated_write', isolationMode: 'isolated_worktree', prompt, externalSessionId: null, onProtocolEvent: (event) => this.runs.appendEvent(spec.run.id, 'agent_protocol', { attemptId, event }) }); spec.setActive(() => spec.adapter.cancel(handle!.handleId)); return await handle.completion; }
  ...
  private async decide(spec: Parameters<DelegatedRunOrchestrator['execute']>[0], modelId: string, phase: 'initial' | 'post_attempt', index: number, result: AgentExecutionResult | null, validation: ValidationStatus, invariant?: string): Promise<LeaderDecision> {
    ...
    const review = await this.reviews.automationEvidence(spec.run.id); const diff = cap(review.patch, 96 * 1024); const commands = this.runs.validationCommands(spec.run.id).filter((item) => phase === 'initial' || item.attemptId === this.runs.attempts(spec.run.id).at(-1)?.id).map((item) => ({ command: item.command, status: item.status, exitCode: item.exitCode, output: cap(item.output, 16 * 1024).value, outputTruncated: item.outputTruncated }));
    const request: LeaderRequest = { protocolVersion: 1, runId: spec.run.id, phase, task: { title: spec.task.title, prompt: spec.task.prompt }, worker: { agentId: spec.run.resolvedAgentId, modelId: spec.run.resolvedModelId }, budget: { attemptIndex: index, maxAttempts: MAX_ATTEMPTS, remainingAttempts: MAX_ATTEMPTS - index }, ...(result ? { attempt: { index, workerResultSummary: result.terminalEvent?.raw ?? null, workerFailureReason: result.failureReason } } : {}), evidence: { gitStatus: cap(review.review.gitStatus, 16 * 1024).value, changedFiles: review.review.changedFiles.slice(0, 500).map((file) => file.path), diff: diff.value, diffTruncated: diff.truncated, validationStatus: validation, validationCommands: commands, priorAttemptSummaries: [...this.runs.attempts(spec.run.id).map((item) => item.resultSummary ?? item.failureReason ?? '').filter(Boolean).slice(-4), ...(invariant ? [invariant] : [])] } };
    const controller = new AbortController(); spec.setActive(() => { controller.abort(); return Promise.resolve(); }); this.runs.appendEvent(spec.run.id, 'leader_request', { phase, attemptIndex: index });
  ...
  private block(runId: string, blocker: string, summary: string): 'blocked' { this.runs.appendEvent(runId, 'autonomous_blocked', { blocker, summary }); return 'blocked'; }
}
