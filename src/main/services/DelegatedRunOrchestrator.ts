...
export const WORKER_TURN_TIMEOUT_MS = 5 * 60 * 1000;
...
export class DelegatedRunOrchestrator {
  public constructor(private readonly runs: RunRepository, private readonly validation: ProjectValidationService, private readonly reviews: RunReviewService, private readonly leader: LeaderClient, private readonly workerTurnTimeoutMs = WORKER_TURN_TIMEOUT_MS) {}
  ...
  public async execute(spec: { run: Run; task: PlannerTask; worktreePath: string; adapter: AgentAdapter; deadline: number; isCancellationRequested: () => boolean; setActive: (cancel: () => Promise<void>) => void; clearActive: () => void }): Promise<RunStatus> {
    ...
    const luna = await this.leader.resolveLuna(); if (spec.isCancellationRequested()) return 'cancelled'; if (Date.now() >= spec.deadline) return 'timed_out'; this.runs.setDelegatedMetadata(run.id, luna.id, MAX_ATTEMPTS, 'validation');
    const baseline = await this.validation.validate(run.id, spec.worktreePath, { deadline: spec.deadline, isCancellationRequested: spec.isCancellationRequested, onProcessStarted: spec.setActive, onProcessFinished: spec.clearActive });
    if (spec.isCancellationRequested()) return 'cancelled'; if (baseline === 'interrupted') return Date.now() >= spec.deadline ? 'timed_out' : 'cancelled';
    if (baseline === 'failed') return this.block(run.id, 'baseline_validation_failed', 'The selected dev/base revision is not green. Resolve its deterministic validation failures before running delegated work.');
    this.runs.setDelegatedMetadata(run.id, luna.id, MAX_ATTEMPTS, 'planning'); this.runs.appendEvent(run.id, 'leader_resolution', { modelId: luna.id });
    ...
    for (let index = 0; index < MAX_ATTEMPTS; index += 1) {
      ...
      const worker = await this.worker(spec, attempt.id, decision.instruction);
      ...
      const failureReason = worker.timedOut ? 'worker_turn_timeout: Worker exceeded the five-minute delegated turn limit.' : result.failureReason;
      if (worker.timedOut) this.runs.appendEvent(run.id, 'worker_turn_timed_out', { attemptId: attempt.id, limitMs: WORKER_TURN_TIMEOUT_MS });
      this.runs.setAttemptStatus(attempt.id, result.succeeded && !worker.timedOut ? 'completed' : 'failed', { finished_at: new Date().toISOString(), external_session_id: result.externalSessionId, result_summary: result.terminalEvent?.raw ?? null, failure_reason: failureReason });
      ...
      if (decision.action === 'BLOCKED') return this.block(run.id, decision.blocker, decision.summary);
      if (decision.action !== 'WORK') return this.block(run.id, 'leader_done_without_validation', 'Leader returned DONE before a passing validation.');
      if (validation === 'passed') {
        ...
        if (decision.workspaceMode === 'checkpoint') {
  ...
  private async worker(spec: Parameters<DelegatedRunOrchestrator['execute']>[0], attemptId: string, prompt: string): Promise<{ result: AgentExecutionResult; timedOut: boolean }> {
    let handle: Awaited<ReturnType<AgentAdapter['startRun']>> | undefined; let cancellationRequested = false; let timedOut = false;
    ...
    try {
      ...
      const workerDeadline = Math.min(spec.deadline, Date.now() + Math.min(this.workerTurnTimeoutMs, WORKER_TURN_TIMEOUT_MS));
      const timer = setTimeout(() => { timedOut = workerDeadline < spec.deadline; void spec.adapter.cancel(handle!.handleId); }, Math.max(0, workerDeadline - Date.now()));
      try { return { result: await handle.completion, timedOut }; } finally { clearTimeout(timer); }
