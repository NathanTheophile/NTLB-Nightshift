...
export class RunReviewService {
  ...
  public async inspect(runId: string): Promise<RunReview> {
    ...
    const review: RunReview = { run, worktreeHead: null, gitStatus: '', changedFiles: [], result: run.resultSummary, failure: run.failureReason, validationStatus: run.validationStatus, batchSteps, activityTotal: this.runs.eventCount(runId, 'activity'), rawProtocolTotal: this.runs.eventCount(runId, 'raw_protocol'), warnings: [] };
