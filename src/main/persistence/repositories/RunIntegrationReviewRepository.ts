...
export class RunIntegrationReviewRepository {
  ...
  public markStale(id: string, reason: string): RunIntegrationReview { this.database.execute("UPDATE run_integration_reviews SET stale_at = ?, stale_reason = ?, integration_status = CASE WHEN integration_status = 'not_started' THEN 'rejected' ELSE integration_status END WHERE id = ?", new Date().toISOString(), reason, id); return this.findRequired(id); }
