...
describe('RunReviewService', () => {
  ...
  it('bounds compacted protocol chunks while preserving fragment timestamp order', async () => {
    const fixture = await setup();
    try {
      for (let index = 0; index < 200; index += 1) {
        fixture.runRepository.appendEvent(fixture.run.id, 'agent_protocol', protocol('thinking.delta', `${index}:${'x'.repeat(1024)}`, 'session-a'), `2026-08-25T00:00:${String(index).padStart(2, '0')}.000Z`);
      }
      const first = fixture.runRepository.listEventPage(fixture.run.id, 'raw_protocol', null, 2);
      const second = fixture.runRepository.listEventPage(fixture.run.id, 'raw_protocol', first.nextCursor, 2);
      expect(first.total).toBeGreaterThan(1); expect(first.events).toHaveLength(2); expect(second.events[0]!.sequence).toBeGreaterThan(first.events.at(-1)!.sequence);
      for (const event of [...first.events, ...second.events]) expect(Buffer.byteLength(JSON.stringify(event.payload))).toBeLessThanOrEqual(64 * 1024);
      const payload = first.events[0]!.payload as { fragments: Array<{ raw: string }>; compaction: { firstTimestamp: string; lastTimestamp: string } };
      expect(payload.fragments.length).toBeGreaterThan(2); expect(payload.fragments[0]!.raw.startsWith('0:')).toBe(true); expect(payload.compaction.firstTimestamp).toBe('2026-08-25T00:00:00.000Z'); expect(payload.compaction.lastTimestamp).toBe(`2026-08-25T00:00:${String(payload.fragments.length - 1).padStart(2, '0')}.000Z`);
    } finally { await fixture.dispose(); }
  });
});
