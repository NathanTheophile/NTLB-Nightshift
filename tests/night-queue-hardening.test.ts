...
describe('Night queue hardening', () => {
  ...
  it('purges a complete terminal follow-up chain and its local worktrees without touching candidate metadata remotely', async () => {
    ...
    const service = new RunService(runs, tasks, workspaces, worktrees, new Map(), { agentId: 'agent', modelId: 'model', timeoutMs: 1_000 });
    await service.purgePlannerTask(task.id);
    expect(removed).toEqual(['C:\\worktrees\\source', 'C:\\worktrees\\follow']); expect(tasks.findById(task.id)).toBeUndefined(); expect(runs.listByTask(task.id)).toEqual([]); expect(database.queryAll('SELECT * FROM run_events')).toEqual([]); database.close();
  });
