...
import { DelegatedLeaderClient, parseLeaderDecision, type LeaderClient, type LeaderDecision, type LeaderRequest } from '../src/main/services/DelegatedLeaderClient';
...
describe('Delegated Leader autonomous correction', () => {
  it('requires an explicit workspace mode for every parsed WORK decision', () => {
    expect(() => parseLeaderDecision('{"protocolVersion":1,"action":"WORK","instruction":"fix it","summary":"repair"}')).toThrow('decision schema');
    expect(parseLeaderDecision('{"protocolVersion":1,"action":"WORK","instruction":"fix it","summary":"repair","workspaceMode":"current"}')).toMatchObject({ action: 'WORK', workspaceMode: 'current' });
  });
  ...
  it('keeps one Run and one worktree while Luna corrects a failed attempt', async () => {
    ...
    try {
      ...
      await writeFile(join(repo, 'package.json'), JSON.stringify({ scripts: { typecheck: "node -e \"const value=require('fs').readFileSync('implementation.txt','utf8');process.exit(value==='base'||value==='fixed'?0:1)\"" } })); await writeFile(join(repo, 'implementation.txt'), 'base'); await mkdir(join(repo, 'node_modules')); await exec('git', ['-C', repo, 'add', '.']); await exec('git', ['-C', repo, 'commit', '-m', 'base']); await exec('git', ['-C', repo, 'branch', '-M', 'dev']); await exec('git', ['-C', repo, 'remote', 'add', 'origin', remote]);
      const db = new DatabaseService(':memory:'); const workspaces = new WorkspaceRepository(db); const tasks = new PlannerTaskRepository(db); const runs = new RunRepository(db); const workspace = workspaces.addOrTouch(repo, 'repo', true); const worker = new CorrectionWorker(); const leader = new QueuedLeader([{ protocolVersion: 1, action: 'WORK', instruction: 'implement', summary: 'start', workspaceMode: 'current' }, { protocolVersion: 1, action: 'WORK', instruction: 'fix validation', summary: 'fix', workspaceMode: 'current' }, { protocolVersion: 1, action: 'DONE', summary: 'validated' }]);
      ...
      expect(tasks.listVisible(workspace.id)).toHaveLength(1); expect(runs.attempts(completed.id)).toHaveLength(2); expect(worker.directories).toEqual([completed.worktreePath, completed.worktreePath]); expect(worker.secondSawBroken).toBe(true); expect(runs.attempts(completed.id).map((attempt) => attempt.validationStatus)).toEqual(['failed', 'passed']); expect(leader.requests[1]!.evidence.validationStatus).toBe('failed'); expect(leader.requests.at(-1)!.budget.remainingAttempts).toBe(10); expect(completed.validationStatus).toBe('passed'); expect(completed.candidatePublishState).toBe('published'); expect(completed.candidateCommitSha).toMatch(/^[0-9a-f]{40}$/); expect((await exec('git', ['--git-dir', remote, 'rev-parse', `refs/heads/${completed.candidateBranchName}`])).stdout.trim()).toBe(completed.candidateCommitSha); expect(service.events(completed.id).events.filter((event) => event.eventType === 'candidate_committed')).toHaveLength(1); expect(service.events(completed.id).events.map((event) => event.eventType)).not.toContain('follow_up_created'); expect(service.events(completed.id).events.some((event) => event.eventType === 'delegated_checkpoint_restored')).toBe(false);
    } finally { await rm(root, { recursive: true, force: true, maxRetries: 3 }); }
  }, 15_000);

  it('blocks before Luna and before a Worker when the clean baseline validation fails', async () => {
    const fixture = await delegatedFixture(new FixedWorker(), new QueuedLeader([{ protocolVersion: 1, action: 'WORK', instruction: 'must not run', summary: 'unused', workspaceMode: 'current' }]), { validationScript: 'node -e "process.exit(1)"' });
    try {
      const run = await fixture.start(); const terminal = await fixture.terminal(run.id);
      expect(terminal.status).toBe('blocked'); expect(terminal.failureReason).toContain('baseline_validation_failed'); expect(fixture.leader.requests).toHaveLength(0); expect(fixture.worker.directories).toHaveLength(0);
      expect(fixture.runs.validationCommands(run.id)).toMatchObject([{ attemptId: null, status: 'failed' }]);
    } finally { await fixture.dispose(); }
  }, 15_000);

  it('cancels an overlong Worker turn without timing out the whole Run', async () => {
    const worker = new HangingWorker(); const fixture = await delegatedFixture(worker, new QueuedLeader([{ protocolVersion: 1, action: 'WORK', instruction: 'work', summary: 'start', workspaceMode: 'current' }, { protocolVersion: 1, action: 'BLOCKED', summary: 'retry with a smaller step', blocker: 'worker turn timed out' }]), { timeoutMs: 5_000, workerTurnTimeoutMs: 20 });
    try {
      const run = await fixture.start(); const terminal = await fixture.terminal(run.id);
      expect(terminal.status).toBe('blocked'); expect(worker.cancelled).toBe(1); expect(fixture.runs.attempts(run.id)[0]?.failureReason).toContain('worker_turn_timeout'); expect(fixture.runs.listEvents(run.id).some((event) => event.eventType === 'worker_turn_timed_out')).toBe(true); expect(terminal.status).not.toBe('timed_out');
    } finally { await fixture.dispose(); }
  }, 15_000);
  ...
  it('checkpoints accepted steps and restores them before a corrective fresh Worker', async () => {
    ...
    const fixture = await delegatedFixture(worker, new QueuedLeader([{ protocolVersion: 1, action: 'WORK', instruction: 'first bounded step', summary: 'start', workspaceMode: 'current' }, { protocolVersion: 1, action: 'WORK', instruction: 'second bounded step', summary: 'continue', workspaceMode: 'current' }, { protocolVersion: 1, action: 'WORK', instruction: 'correct only the rejected step', summary: 'repair', workspaceMode: 'checkpoint' }, { protocolVersion: 1, action: 'DONE', summary: 'done' }]), { validationScript: "node -e \"const value=require('fs').readFileSync('implementation.txt','utf8');process.exit(value==='rejected'?1:0)\"" });
    try {
      const run = await fixture.start(); const terminal = await fixture.terminal(run.id);
...
const delegatedFixture = async <TLeader extends LeaderClient>(worker: CorrectionWorker, leader: TLeader, options: { timeoutMs?: number; validationScript?: string; sourceDependencies?: boolean; modelId?: string; workerTurnTimeoutMs?: number } = {}) => {
  const root = await mkdtemp(join(tmpdir(), 'nightshift-delegated-cases-')); const repo = join(root, 'repo'); const remote = join(root, 'remote.git'); await exec('git', ['init', repo]); await exec('git', ['init', '--bare', remote]); await exec('git', ['-C', repo, 'config', 'user.email', 'test@nightshift']); await exec('git', ['-C', repo, 'config', 'user.name', 'test']); await writeFile(join(repo, 'package.json'), JSON.stringify({ scripts: { typecheck: options.validationScript ?? "node -e \"const value=require('fs').readFileSync('implementation.txt','utf8');process.exit(value==='base'||value==='fixed'?0:1)\"" } })); await writeFile(join(repo, 'implementation.txt'), 'base'); await exec('git', ['-C', repo, 'add', '.']); await exec('git', ['-C', repo, 'commit', '-m', 'base']); await exec('git', ['-C', repo, 'branch', '-M', 'dev']); await exec('git', ['-C', repo, 'remote', 'add', 'origin', remote]);
  if (options.sourceDependencies !== false) await mkdir(join(repo, 'node_modules'));
  const modelId = options.modelId ?? 'model'; const db = new DatabaseService(':memory:'); const workspaces = new WorkspaceRepository(db); const tasks = new PlannerTaskRepository(db); const runs = new RunRepository(db); const workspace = workspaces.addOrTouch(repo, 'repo', true); const supervisor = new WindowsProcessSupervisor(); const delegated = new DelegatedRunOrchestrator(runs, new ProjectValidationService(runs, supervisor), new RunReviewService(runs, workspaces), leader, options.workerTurnTimeoutMs); const service = new RunService(runs, tasks, workspaces, new GitWorktreeService(join(root, 'worktrees')), new Map([[worker.id, worker]]), { agentId: worker.id, modelId, timeoutMs: options.timeoutMs ?? 10_000 }, supervisor, undefined, delegated);
