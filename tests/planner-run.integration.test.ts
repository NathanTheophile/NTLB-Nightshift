...
describe.skipIf(!enabled)('Planner → Run real FCC probe', () => {
  it('keeps the user workspace untouched and persists the isolated Claude result', async () => {
    ...
    try {
      ...
      expect(await readFile(join(repository, 'README.md'), 'utf8')).toBe('base\n'); expect(await readFile(join(completed.worktreePath!, 'NIGHTSHIFT_AGENT_PROBE.txt'), 'utf8')).toBe('NIGHTSHIFT_AGENT_PROBE_OK'); expect(completed.externalSessionId).toBeTruthy(); expect(completed.baseSha).toBeTruthy(); expect(completed.finalGitState).toBeTruthy(); expect(service.events(completed.id).some((event) => event.eventType === 'claude_protocol')).toBe(true);
